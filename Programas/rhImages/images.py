...
class Reader:
    ...
    def read(self, mode, bitdepth = -1):
        ...
        if self.file.name.split('.')[-1].lower() == 'png':
            ...
            w.read_header()
            if bitdepth == -1: 
                bitdepth = w.infoheader['bitdepth']
            ...
            self.width = w.infoheader['width']
            self.height = w.infoheader['height']
        ...
        if isinstance( self.colormap[0][0], int ):
            self.colormap = map(lambda x: map(lambda y:y/255.0, x), self.colormap)

        return self.data, self.colormap

    def as_data(self, mode, bitdepth = -1, palette = None):
        ...
        if mode == 1:
            ...
            width_tiles = (len(self.data[0]) / self.bitdepth)
            ...
            for row in self.data:
                ...
                if self.bitdepth < 8:
                    ...
                    row = map(lambda e: reduce(lambda x,y:(x << self.bitdepth) + y, reversed(e)), row)
                number = 0
                for tile in tiles_row:
                    tile += row[(number*self.bitdepth):((number+1)*self.bitdepth)]
                    number += 1
                height += 1
                if height == 8:
                    height = 0
        ...
        return array.array('c', bitarray.tostring()), self.colormap[:2**self.bitdepth]
